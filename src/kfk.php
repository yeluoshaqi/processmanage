...
class Kfk {
	...
	public function __construct($config = []) {
		$this->topic = isset($config['topic']) ? $conf['topic'] : "";
		$this->group = isset($config['group']) ? $conf['group'] : "";
		$this->brokers = isset($config['brokers']) ? $conf['brokers'] : "";
		if(empty($this->topic) || empty($this->group) || empty($this->brokers)) {
			throw new Exception("kafka 参数错误", 1);
		}
