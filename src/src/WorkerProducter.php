...
class WorkerProducter extends Process {
	...
	public function __construct($config = []) {
		...
		$this->queueName = isset($config['queue_name']) ? $config['queue_name'] : "";
		$this->queueConfig = isset($config["queue_config"]) ? $config["queue_config"] : [];
		parent::__construct();
	}
	...
	private function initQueue() {
		...
		switch ($this->queueName) {
			case 'kafka':
				...
				break;
			case 'redis':
				$this->queue = new Redis($this->queueConfig);
				break;
			default:
